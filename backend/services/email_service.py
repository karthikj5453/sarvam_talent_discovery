...
def _send(to_email: str, subject: str, html_body: str) -> bool:
    ...
    if not _is_configured():
        logger.info(f"[Email] SMTP not configured — skipping email to {to_email}: {subject}")
        return False
    try:
        ...
        logger.warning(f"[Email] ⚠️ Failed to send email to {to_email}: {e}")
        return False
...
def send_application_received(candidate_name: str, candidate_email: str, job_title: str) -> bool:
    ...
    <div style="font-family:Inter,Arial,sans-serif;max-width:560px;margin:0 auto;padding:32px;background:#0f172a;color:#e2e8f0;border-radius:12px;">
      ...
      <p style="color:#94a3b8;line-height:1.6;">The system will evaluate you across <strong>7 competency dimensions</strong> using your voice responses — in any Indian language you're comfortable with.</p>
      <div style="background:#1e293b;border-radius:8px;padding:16px;margin:24px 0;border-left:4px solid #6366f1;">
        <p style="color:#c7d2fe;margin:0;font-size:14px;">🌐 <strong>Speak naturally</strong> — Hindi, Tamil, Telugu, Kannada, Malayalam, Marathi, Bengali, Gujarati, or Punjabi are all supported.</p>
      </div>
